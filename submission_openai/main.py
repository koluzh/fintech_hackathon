...
class LLMChunker:
    ...
    async def chunk_corpus(self):
        ...
        for result in results:
            for x in result:
                x_dict = x.dict()
                self.chunks.append(self.dict_to_xml(x_dict))
...
class MyKAG:
    def __init__(self, client, model_name, store, reranker_api_key, reranker_top_k=1, reranker_threshold=0.0, truncation_limit=1500):
        self.client = client
        self.model_name = model_name
        self.store = store
        self.queries = []
    ...
    def get_knowledge(self, query):
        docs = self.store.similarity_search(query=str(query.dict()), k=10)
...
async def main(questions, vectorstore, reranker_api_key, client, model_name):
    kag = MyKAG(
        client=client,
